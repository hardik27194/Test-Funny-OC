//
//  ContentRecommendViewController.m
//  Funny
//
...
#import "ContentRecommendViewController.h"
#import "ContentVideoTableViewCell1.h"
#import "FunnyVideoPlayManage.h"
#import "WindowViewManager.h"

@interface ContentRecommendViewController ()<VideoPlayDelegate>

@end

@implementation ContentRecommendViewController
...
- (NSString *)netUrl:(eRefresh)refresh
{
    ...
    if (refresh == kNormalrefresh) {
        urlString=ContentRecommendDefaultURL;
    }else if (refresh == kPullRefresh){
        urlString=[ContentRecommendMaxHeadURL stringByAppendingFormat:@"%lld", self.maxTime];
        urlString=[urlString stringByAppendingString:ContentRecommendMaxFootURL];
    }else if(refresh == kPushRefresh){
        urlString=[ContentRecommendMinHeadURL stringByAppendingFormat:@"%lld", self.minTime];
        urlString=[urlString stringByAppendingString:ContentRecommendMinFootURL];
    }
    return urlString;
}
...
-(void)netRequestWithRefresh:(eRefresh)refresh baseView:(YZRefreshBaseView *)baseView isHeaderRefresh:(BOOL)end{
    ...
    [NetManager requestDataWithURLString:urlString contentType:JSON finished:^(id responseObj) {
        ...
        for (NSDictionary *dict in dataArray) {
            ...
            //int type=[groupDict[@"type"] intValue]; //==3
            NSString *category_name=groupDict[@"category_name"];
            if (![category_name isEqualToString:@"搞笑视频"]) {
                continue;
            }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    ContentWebViewController *vc=[[ContentWebViewController alloc] initWithUrlString:model.share_url];
